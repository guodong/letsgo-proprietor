    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <div class="radio sale-state">
          <label><input name="sale_state" type="radio" value="1" :checked="state" @change="checkState">上架</label>
          <label><input name="sale_state" type="radio" value="0" :checked="!state" @change="checkState">下架</label>
        </div>
      </div>
    </div>
    <div class="form-group">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      inputPrice (price) {
        this.$emit('inputPrice', { index: this.index, price, values: this.values })
      },
      inputUnit (unit) {
        this.$emit('inputUnit', { index: this.index, unit, values: this.values })
      },
      inputStock (stock) {
        this.$emit('inputStock', { index: this.index, stock, values: this.values })
      },
      checkState (event) {
